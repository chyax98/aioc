...
const (
	aiocCodexMcpBeginMarker = "# BEGIN aioc-managed mcp_servers (do not edit; regenerated by daemon)"
	aiocCodexMcpEndMarker   = "# END aioc-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(aiocCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(aiocCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = aiocCodexMcpBeginMarker + "\n" + aiocCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(aiocCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(aiocCodexMcpEndMarker)
	sb.WriteString("\n")
