...
// the aioc CLI version reported by the daemon at registration time. The
...
// `aioc issue create` failure — see PR #1851); older daemons would either
...
var (
	ErrCLIVersionMissing = errors.New("aioc CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("aioc CLI version is below required minimum")
)
